...
async function resolveUri() {
  if (process.env.NODE_ENV === 'production' && !process.env.MONGO_URI) {
    // Never let a deployed environment silently fall back to a throwaway database.
...
async function ensureIndexes(d) {
  ...
  await d.collection('feed_records').createIndex({ date: -1 });
  await d.collection('feed_records').createIndex({ feedType: 1, date: -1 });
  await d.collection('sales').createIndex({ date: -1 });
  await d.collection('purchases').createIndex({ date: -1 });
  await d.collection('transactions').createIndex({ date: -1 });
  await d.collection('transactions').createIndex({ kind: 1, category: 1, date: -1 });
}
